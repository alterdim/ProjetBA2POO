...
//    }
    ...
    private static final class TestPlayerPerso implements Player {
        ...
        public TestPlayerPerso(long randomSeed, List<Route> allRoutes) {
            this.rng = new Random(randomSeed);
            this.allRoutes = List.copyOf(allRoutes);
            turnCounter = 0;
            receiveCounter = 0;
        ...
        @Override
        public SortedBag<Ticket> chooseInitialTickets() {
            SortedBag.Builder<Ticket> ticketslists =  new SortedBag.Builder();
            for(Ticket ticket:tickets){
                if (new Random().nextInt(5)>3 || ticketslists.size()<3) ticketslists.add(ticket);
            }
            return ticketslists.build();
        }
...
//                    System.out.println(r.id());
        ...
        @Override
        public SortedBag<Ticket> chooseTickets(SortedBag<Ticket> options) {
            SortedBag.Builder ticketslists =  new SortedBag.Builder();
            for(Ticket ticket : options){
                if (new Random().nextInt(5)>2 || ticketslists.size()<1) ticketslists.add(ticket);
            }
            return ticketslists.build();
        }
...
//                System.out.println(options.get(0));
    ...
    private static final class ChMapPerso {
        //region Stations
